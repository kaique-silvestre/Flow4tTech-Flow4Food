...
- [ ] **A1.** `models/comandas.py` — adicionar ao `StatusComanda`:
  ```python
  REABERTA = "reaberta"
  ```

- [ ] **A2.** `models/eventos_comanda.py` — adicionar ao `TipoEvento`:
  ```python
  COMANDA_REABERTA = "comanda_reaberta"
  ```

- [ ] **A3.** `models/movimentos_estoque.py` — adicionar ao `TipoMovimento`:
  ```python
...
- [ ] **B1.** `core/errors.py` — adicionar ao `ErrorCode`:
  ```python
...
- [ ] **C1.** `repositories/comandas_repository.py` — modificar `list_abertas` para incluir `REABERTA`:
  ```python
...
- [ ] **C2.** `repositories/comandas_repository.py` — adicionar `list_fechadas`:
  ```python
...
- [ ] **C3.** `repositories/comandas_repository.py` — adicionar `reabrir_comanda_repo`:
  ```python
...
- [ ] **D1.** `services/comandas_service.py` — adicionar `reabrir_comanda`:
  ```python
...
- [ ] **D2.** `services/comandas_service.py` — adicionar `_estornar_estoque`:
  ```python
...
- [ ] **E1.** `api/routes/comandas.py` — adicionar 2 endpoints:
  ```python
...
- [ ] **F1.** `tests/test_reabertura.py` (6 testes):
...
- [ ] **G1.** `features/comandas/useComandas.ts` — adicionar:
  ```ts
...
- [ ] **H1.** `features/comandas/ComandasPage.tsx` — adicionar seção "Histórico":
...
- [ ] **I1.** `features/comandas/ComandaAbertaPage.tsx` — adicionar Reabrir quando `comanda.status === "fechada"`:
...
- [ ] `StatusComanda.REABERTA = "reaberta"` adicionado.
- [ ] `TipoEvento.COMANDA_REABERTA` adicionado.
- [ ] `TipoMovimento.ENTRADA_ESTORNO` adicionado.
- [ ] `ErrorCode.COMANDA_NAO_FECHADA` adicionado.
- [ ] `POST /api/comandas/{id}/reabrir` só aceita `status=fechada`; 400 `COMANDA_NAO_FECHADA` caso contrário.
- [ ] Transação atômica: estorno + status REABERTA + evento numa única `db.commit()`.
- [ ] Item simples: 1 movimento `ENTRADA_ESTORNO`, estoque incrementado.
- [ ] Item composto: 1 movimento por insumo da ficha técnica.
- [ ] `list_abertas` retorna também `reaberta`.
- [ ] `GET /api/comandas/fechadas` retorna lista de fechadas.
- [ ] Testes: 6 cenários passando.
- [ ] FE: seção Histórico em ComandasPage com fechadas.
- [ ] FE: botão Reabrir em ComandaAbertaPage quando status=fechada, com modal de confirmação.
- [ ] FE: após reabertura, navigate para `/comandas/:id` (volta ao fluxo normal).
