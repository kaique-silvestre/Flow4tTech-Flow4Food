...
class ComissaoGarcom(Base):
    ...
    garcom_id: Mapped[int] = mapped_column(sa.ForeignKey("garcons.id"), nullable=False, index=True)
    comanda_id: Mapped[int] = mapped_column(sa.ForeignKey("comandas.id"), nullable=False, index=True)
