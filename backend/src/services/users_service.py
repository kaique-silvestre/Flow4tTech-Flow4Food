...
def get_users(
    db: Session, tenant_id: int, search: Optional[str] = None, profile_id: Optional[int] = None
) -> list[UserResponse]:
    users = list_users(db, tenant_id, search, profile_id)
    return [_to_response(u) for u in users]


def get_user(db: Session, tenant_id: int, user_id: int) -> UserResponse:
...
def update_existing_user(
    db: Session, tenant_id: int, user_id: int, data: UserUpdate, current_user_id: int
) -> UserResponse:
    user = get_user_by_id(db, user_id)
    if not user or user.tenant_id != tenant_id:
        raise AppError(code=ErrorCode.NOT_FOUND, message="Usuário não encontrado", http_status=404)
...
def toggle_user_active(db: Session, tenant_id: int, user_id: int, current_user_id: int) -> UserResponse:
...
def delete_existing_user(db: Session, tenant_id: int, user_id: int, current_user_id: int) -> None:
    if user_id == current_user_id:
...
def reset_password(db: Session, tenant_id: int, user_id: int) -> str:
    user = get_user_by_id(db, user_id)
    if not user or user.tenant_id != tenant_id:
        raise AppError(code=ErrorCode.NOT_FOUND, message="Usuário não encontrado", http_status=404)
...
def check_username_available(db: Session, tenant_id: int, username: str) -> bool:
