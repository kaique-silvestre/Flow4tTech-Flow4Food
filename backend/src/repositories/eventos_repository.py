...
def update(db: Session, evento: TenantEvento, data: EventoPatch) -> TenantEvento:
    ...
    if data.hora_inicio is not None or "hora_inicio" in (data.model_fields_set or set()):
        evento.hora_inicio = data.hora_inicio
    if data.hora_fim is not None or "hora_fim" in (data.model_fields_set or set()):
        evento.hora_fim = data.hora_fim
