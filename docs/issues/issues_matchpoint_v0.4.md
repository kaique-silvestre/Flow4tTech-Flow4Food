...
- [ ] Abrir modal de nova comanda → radio "Nome do responsável" já está selecionado
- [ ] "Nome do responsável" aparece antes de "Número da mesa" na lista
- [ ] Input de identificação abre como campo de texto (não numérico)
- [ ] Trocar para "Número da mesa" continua funcionando normalmente
- [ ] Criar comanda por nome → backend recebe `tipo_identificacao: "nome"`
- [ ] Criar comanda por mesa → backend recebe `tipo_identificacao: "mesa"` com número inteiro
...
- [ ] Desativar produto X → abrir comanda → buscar produto X → não aparece nos resultados
- [ ] Produto X reativado → busca na comanda → aparece normalmente
- [ ] `CardapioPage` continua exibindo produtos inativos no filtro "Inativos"
- [ ] `GET /api/produtos?ativo=true` retorna apenas produtos com `ativo=true`
- [ ] `GET /api/produtos` sem parâmetro continua retornando todos (sem quebra de comportamento existente)
...
- [ ] Toast de erro aparece e desaparece automaticamente após ~8 segundos
- [ ] Toast de sucesso desaparece após ~2,5 segundos (sem regressão)
- [ ] Toast de aviso desaparece após ~4,5 segundos (sem regressão)
- [ ] Clicar em qualquer toast fecha imediatamente (comportamento padrão mantido)
- [ ] Nenhum toast fica preso na tela indefinidamente
...
- [ ] Produto na subcategoria "Sucos" (filha de "Bebidas") → coluna exibe "Bebidas > Sucos"
- [ ] Produto na categoria raiz "Sobremesas" → coluna exibe "Sobremesas"
- [ ] Produto sem categoria → coluna exibe "—"
- [ ] Filtro de categoria na barra superior continua funcionando corretamente
- [ ] Layout da tabela não quebra com nomes longos de hierarquia
...
- [ ] Digitar `1050` no campo de preço → exibe `R$ 1.050,00`
- [ ] Digitar `10` → exibe `R$ 10,00` (duas casas fixas)
- [ ] Digitar `0,50` → exibe `R$ 0,50`
- [ ] Valor submetido ao backend é número sem máscara (não inclui "R$ ")
- [ ] Campo de preço de venda no `ProdutoModal` usa `MoneyInput`
- [ ] Campos de custo total e custo unitário em `NovaCompraPage` usam `MoneyInput`
- [ ] Cálculo de total da compra continua correto após troca do input
...
- [ ] Salvar produto sem categoria → erro inline aparece abaixo do select ("Selecione uma categoria")
- [ ] Select de categoria fica destacado em vermelho quando inválido
- [ ] Nenhuma chamada à API é disparada com categoria vazia
- [ ] Selecionar uma categoria → erro some imediatamente
- [ ] Editar produto existente com categoria → formulário abre sem erro
- [ ] Criar produto com categoria válida → salva normalmente
...
- [ ] Migration aplicada sem erro em banco existente
- [ ] Métodos existentes têm `tipo="outro"` após migration
- [ ] Modal de criar/editar método exibe select de tipo com as 5 opções
- [ ] `GET /api/metodos-pagamento` inclui campo `tipo` no response
- [ ] Criar método "Dinheiro" com tipo "dinheiro" → response confirma `tipo: "dinheiro"`
- [ ] Editar método existente → campo tipo preenchido com valor atual
...
- [ ] Migration de `valor_nota` e `troco` aplicada sem erro
- [ ] Selecionar método com `tipo="dinheiro"` na tela de fechamento → campos de nota e troco aparecem
- [ ] Selecionar método com outro tipo → campos de nota/troco não aparecem
- [ ] Digitar valor da nota → troco calculado em tempo real (`nota - valor_pagamento`)
- [ ] Nota menor que o total → aviso vermelho "Nota insuficiente"
- [ ] Fechar comanda sem preencher valor da nota → fecha normalmente (campo opcional)
- [ ] Fechar com nota R$50 para total R$37,50 → `valor_nota=50`, `troco=12.50` persistidos no banco
- [ ] Comprovante com troco > 0 exibe "Valor recebido: R$ 50,00" e "Troco: R$ 12,50"
- [ ] Comprovante sem troco não exibe essas linhas
...
- [ ] Migration aplicada sem erro — todos os insumos existentes com `estoque_reservado=0`
- [ ] Adicionar hambúrguer (usa 2 pães) com estoque de pão = 1 → item adicionado + toast "Estoque insuficiente: Pão de hambúrguer"
- [ ] Adicionar hambúrguer com estoque de pão = 5 → item adicionado sem toast
- [ ] `estoque_reservado` do pão incrementa em 2 após adição do hambúrguer
- [ ] Cancelar o item de hambúrguer → `estoque_reservado` do pão decrementa em 2
- [ ] `estoque_reservado` nunca fica negativo (piso em zero)
- [ ] Cancelar comanda inteira → `estoque_reservado` de todos os insumos retorna ao valor anterior
- [ ] Fechar comanda → `estoque_atual` debitado E `estoque_reservado` decrementado (ambos)
- [ ] Pagamento parcial → nem `estoque_atual` nem `estoque_reservado` são alterados
- [ ] Comanda com produto sem ficha técnica → adição não gera erro, `estoque_insuficiente` vazio
...
- [ ] `GET /api/insumos` inclui `estoque_reservado` e `estoque_disponivel` no response
- [ ] Tela de estoque exibe colunas "Estoque atual", "Reservado" e "Disponível"
- [ ] Insumo com `estoque_atual=5`, `estoque_reservado=3` → "Disponível" exibe 2
- [ ] Insumo com `estoque_atual=1`, `estoque_reservado=3` → "Disponível" exibe -2 em vermelho
- [ ] Insumo sem reservas → "Reservado" exibe 0, "Disponível" igual a "Estoque atual"
- [ ] Layout da tabela não quebra com as novas colunas
...
- Bloco 2: issue 8 (após #7 concluída), issue 10 (após #9 concluída)
