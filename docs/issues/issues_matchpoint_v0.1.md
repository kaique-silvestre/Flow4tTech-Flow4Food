...
- [x] Card "Lucro estimado" removido do Dashboard
- [x] Heatmap de dias do mês removido
- [x] Tabs `[Resumo]` e `[Histórico]` presentes
- [x] Tab Histórico exibe dados de entrada/saída por dia com filtro de período
- [x] Calendário anual exibe 12 meses com faturamento e gasto por mês
- [x] Meses sem dados exibem "—" (não R$0)
- [x] `GET /api/dashboard/historico?inicio=&fim=` retorna `[{data, faturamento, total_compras}]`
- [x] `GET /api/dashboard/resumo-anual?ano=` retorna array de 12 entradas `{mes, faturamento, total_compras}`
- [x] Mês atual no calendário reflete dados em tempo real
- [x] Teste: `GET /api/dashboard/resumo-anual` → exatamente 12 entradas
...
- [x] Migration: `parent_id INTEGER NULLABLE REFERENCES categorias(id)`
- [x] Criar subcategoria de categoria raiz: OK
- [x] Criar subcategoria de subcategoria: retorna erro 422 (3º nível bloqueado)
- [x] `DELETE /api/categorias/:id` com filhos ativos retorna 409
- [x] `GET /api/categorias` retorna estrutura hierárquica com `children`
- [x] `CategoriasPage` exibe hierarquia em accordion expansível
- [x] `CategoriaModal`: campo opcional "Categoria pai" (Select com apenas categorias raiz)
- [x] Seletores em modais de insumo/produto: subcategorias indentadas
- [x] Insumos e produtos podem ser vinculados a subcategorias
